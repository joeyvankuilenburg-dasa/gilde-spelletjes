...
import type { NotebookContext } from './mascotSprites';
...
export function SamExplains({
  title = 'Sam legt het uit',
  intro,
  notebookContext,
  steps,
}: SamExplainsProps) {
  return (
    <div className="relative flex items-start gap-3 rounded-card bg-surface p-4 shadow-card">
      ...
      <div className="flex shrink-0 flex-col items-center gap-1">
        <SamAvatar notebookContext={notebookContext} className="h-24 w-24 sm:h-28 sm:w-28" />
        <span className="text-[10px] font-extrabold uppercase tracking-widest text-muted">Sam</span>
      </div>
